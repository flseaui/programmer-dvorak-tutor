...
fn run(rx: mpsc::Receiver<Arc<Lesson>>, tx: mpsc::Sender<Arc<Lesson>>) -> JoinHandle<()> {
    let mut lessons = vec![];
    thread::spawn(move || loop {
        if lessons.is_empty() {
            lessons.push(rx.recv().unwrap());
        }
        for lesson in &lessons.pop() {
            println!("next lesson: {}", lesson.id);
            let again = run_lesson(&lesson);
            save_stats(lesson.deref().clone());
            println!("u have no idea: {}", *again);
            if *again {
                println!("we are gaming dont cha know");
                println!("1");
                println!("2");
                let index = lessons_lock.entry(lesson.id.clone()).index() + 1;
                println!("3");
                let lesson_new = lessons_lock.get_index(index).unwrap().1.clone();
                println!("4");
                lessons.push(lesson_new);

                println!("CHAHCHHCHAHCHACHCA");
            } else {
                return;
            }
        }
...
pub fn run_lesson(lesson: &Lesson) -> Box<bool> {
